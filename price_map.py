...
def create_map(df):
    ...
    df = df.copy()
    df.dropna(subset=["geom", "prix_valeur", "com_code", "dep_code"], inplace=True)
    df = df[["geom", "prix_valeur", "com_code", "dep_code"]]
    ...
    use_dep_code = st.checkbox("Utilise Code du Département au lieu de Code de Commune")
    # Add checkbox for choosing between mean and median
    use_median = st.checkbox("Utilise Médiane au lieu de Moyenne")
    # Checkboxes logic
    ...
    st.map(df, latitude="lon", longitude="lat", color="color")  # Fixed lat and lon, reversed in st.map for some reason
